...
function PartnerDashboard() {
  ...
  const fetchPatients = async () => {
    try {
      ...
      const data = response.data.results || response.data; // Handle both paginated and non-paginated
      ...
      let mapped = (Array.isArray(data) ? data : []).map(p => ({
        id: p.patient_id,
      ...
      >
        <form onSubmit={handleAddPatientSubmit} className="flex flex-col flex-1 overflow-hidden">
          ...
          <div className="px-6 py-5 border-t border-gray-100 flex justify-end gap-4 flex-shrink-0 bg-gray-50">
            ...
            <button type="submit" disabled={isSubmitting} className={`px-6 py-2 bg-[#00acc1] hover:bg-[#0097a7] text-white font-semibold rounded-lg shadow transition-colors ${isSubmitting ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}>
