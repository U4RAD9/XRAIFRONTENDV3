  ...
  const handleCheckoutSubmit = async () => {
    ...
    try {
      ...
      if (response.data.Success === true) {
        ...
        setShowConsentModal(false);
        setConsentChecked(false);
        navigate('/admin/make-booking');
        window.location.reload(); // Quick refresh to reset the state since it's the same page
      } else {
      ...
      <div className="w-full">
        ...
        <div className="bg-white shadow-sm border border-gray-200 rounded-lg p-6 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            {/* The Payment Mode logic from here is moved to Cart Modal */}
          </div>
      ...
      >
        <form onSubmit={handleAddPatientSubmit} className="flex flex-col flex-1 overflow-hidden">
          ...
          <div className="px-6 py-5 border-t border-gray-100 flex justify-end gap-4 flex-shrink-0 bg-gray-50">
            <button type="button" onClick={() => setShowAddPatientModal(false)} className="px-6 py-2 border border-gray-300 text-gray-600 font-semibold hover:bg-gray-100 rounded-lg transition-colors cursor-pointer">
              Cancel
            </button>
...
export default AdminMakeBooking;
