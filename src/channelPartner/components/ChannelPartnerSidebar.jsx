...
import { channelPartnerNavLinks } from './channelPartnerNavLinks';

function ChannelPartnerSidebar({ isSidebarOpen, setIsSidebarOpen, handleLogout }) {
    ...
    <>
      ...
      <div className="w-64 shrink-0 bg-[#233560] text-white hidden md:flex flex-col shadow-xl z-40 sticky top-0 h-screen overflow-y-auto">
        ...
        <nav className="flex-1 px-4 py-6 space-y-1">
          {channelPartnerNavLinks.map((link, index) => (
            <Link
              key={index}
              to={link.path}
        ...
        <div className="p-4 border-t border-gray-700">
          <button onClick={handleLogout} className="w-full flex items-center px-4 py-2 text-gray-300 hover:bg-gray-800 hover:text-white rounded-lg transition-colors">
