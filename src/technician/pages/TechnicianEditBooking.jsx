    ...
    <div className="w-full">
      ...
      <div className="mb-8 bg-white shadow-sm border border-gray-200 rounded-lg p-4 md:p-6">
        <div className="grid grid-cols-12 gap-4 items-end mb-6">
          ...
          <div className="col-span-12 md:col-span-3">
            <label className="block text-xs font-semibold text-gray-600 mb-1 text-center md:text-left uppercase">Body Part</label>
            <select
          ...
          <div className="col-span-12 md:col-span-2">
            <label className="block text-xs font-semibold text-gray-600 mb-1 text-center md:text-left uppercase">Price</label>
            <input
              type="number"
          ...
          <div className="col-span-12 md:col-span-3">
            <label className="block text-xs font-semibold text-gray-600 mb-1 text-center md:text-left uppercase">Net Payable</label>
            <input
              type="number"
        ...
        {serviceItems.length > 0 && (
          <div className="mb-6 space-y-4 md:space-y-0">
            {serviceItems.map((item, index) => (
              <div key={index} className="grid grid-cols-12 gap-4 items-center mb-2 p-3 md:py-3 md:px-0 border border-gray-100 md:border-b md:border-x-0 md:border-t-0 rounded-lg md:rounded-none bg-gray-50 md:bg-white">
                <div className="col-span-12 md:col-span-3 text-sm text-gray-700 md:px-2">
                  <span className="md:hidden font-semibold">Service: </span>{item.serviceName}
                </div>
                <div className="col-span-12 md:col-span-3 text-sm text-gray-700 md:px-2">
                  <span className="md:hidden font-semibold">Body Part: </span>{item.bodyPartName}
                </div>
                <div className="col-span-12 md:col-span-2 text-sm text-gray-700 md:px-2">
                  <span className="md:hidden font-semibold">Price: </span>{item.price}
                </div>
                <div className="col-span-12 md:col-span-3 text-sm text-gray-700 md:px-2">
                  <span className="md:hidden font-semibold">Net Payable: </span>{item.netPayable}
                </div>
                <div className="col-span-12 md:col-span-1 md:text-center mt-2 md:mt-0 flex justify-end">
                  <button
                    onClick={() => {
      ...
      {showConsentModal && (
        <div className="fixed inset-0 bg-black/60 flex justify-center items-center z-[70]" onClick={() => { setShowConsentModal(false); setConsentChecked(false); }}>
          <div className="bg-white rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-4 md:p-6 overflow-y-auto flex-1 text-gray-600 text-sm leading-relaxed space-y-4">
            ...
            <div className="p-4 border-t border-gray-200 flex flex-col md:flex-row justify-end gap-3 md:gap-4 bg-gray-50">
              <button
                disabled={!consentChecked}
                onClick={handleCheckoutSubmit}
                className={`w-full md:w-auto px-8 py-2 font-bold rounded shadow-sm transition-colors uppercase ${consentChecked ? 'bg-[#d63384] hover:bg-[#c82375] text-white cursor-pointer' : 'bg-gray-300 text-gray-500 cursor-not-allowed'}`}
              >
                Submit
              </button>
              <button
                onClick={() => { setShowConsentModal(false); setConsentChecked(false); }}
                className="w-full md:w-auto px-6 py-2 border border-gray-300 text-gray-600 font-bold rounded shadow-sm hover:bg-white transition-colors bg-white uppercase cursor-pointer"
              >
                Close
              </button>
            </div>
