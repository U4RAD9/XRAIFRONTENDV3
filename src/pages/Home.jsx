...
//                       <div
//                         className="w-full h-full bg-center bg-no-repeat bg-[#11A8A4] bg-[length:100%_100%] sm:bg-contain"
...
//             className={`px-5 py-2 border border-[#11A8A4] rounded cursor-pointer text-center font-bold transition-colors ${activeStep === step ? 'bg-[#11A8A4] text-white' : 'text-[#11A8A4] hover:bg-[#11A8A4] hover:text-white'
//               }`}>
//             Step {step}
//           </div>
//         ))}
...
import React, { useState } from 'react';
import { Helmet } from 'react-helmet-async'; // npm install react-helmet-async, then wrap <App/> in <HelmetProvider>
import { Link } from 'react-router-dom';
import PackageCard from '../components/PackageCard';
import { Swiper, SwiperSlide } from 'swiper/react';
import { Navigation, Pagination, Autoplay } from 'swiper/modules';
...
function Home() {
  ...
  return (
    <div className="bg-[#f8f9fa] w-full font-sans">
      ...
      <Helmet>
        ...
        <script type="application/ld+json">
          {JSON.stringify({
            ...
            "name": "XRAi Digital",
            "url": "https://xraidigital.com",
            "telephone": "+91-1800-270-2900",
            ...
            "makesOffer": [
              {
                "@type": "Offer",
                "itemOffered": { "@type": "MedicalProcedure", "name": "X-Ray at Home" },
              },
              {
                "@type": "Offer",
      ...
      {/* Available Cities — real visible text (not an image) carrying the
          target keywords, so Google indexes them as page content */}
      <div className="container mx-auto mt-12 mb-6 px-6">
        <h2 className="text-2xl md:text-3xl font-bold text-black mb-6">
          <i className="fa fa-map-marker-alt text-[#0F847E] mr-2"></i> Where We Provide At-Home Diagnostics
        </h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          <div className="bg-white rounded-xl shadow-md p-5">
            <h3 className="text-lg font-bold text-gray-800 mb-3">X-Ray at Home</h3>
            <ul className="space-y-1 text-sm font-semibold text-gray-600">
              {SERVICE_CITIES.map((city) => (
                <li key={`xray-${city}`}>X-Ray at Home in {city}</li>
              ))}
            </ul>
          </div>
          <div className="bg-white rounded-xl shadow-md p-5">
            <h3 className="text-lg font-bold text-gray-800 mb-3">ECG at Home</h3>
            <ul className="space-y-1 text-sm font-semibold text-gray-600">
              {SERVICE_CITIES.map((city) => (
                <li key={`ecg-${city}`}>ECG at Home in {city}</li>
              ))}
            </ul>
          </div>
        </div>
